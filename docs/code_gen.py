...
sheet_names = ['OAuth인증', '국내주식주문', '국내주식시세', '국내주식실시간', '국내선물옵션주문', '국내선물옵션시세', '해외주식주문', '해외주식현재가']
sheet = wb[sheet_names[2]]


def extract_api_row_index():
    api_row_nums = []
    count = 0
...
def extract_api_parameters(name):
    ...
    for row in sheet.iter_rows(values_only=False):
        ...
        if _found["Request"] and row[0].value == "tr_id" and row[5].value == "[실전/모의투자]":
            # print(f"{row[0].value}")
            _header[f"{row[0].value}"] = f"{sheet[row[0].row + 1][5].value}".split(":")[0].strip()
...
def generate_code(api_summary, header):
    ...
    print(f'    let query = [("fid_cond_mrkt_div_code", "J"), ("fid_input_iscd", ticker)];')
    print(f'    let req = self.make_request(url, RequestType::{api_summary["Method"]}, &headers, &query)?;')
    print('')
...
if __name__ == '__main__':
    apis_name_list = ["주식현재가 시세[v1_국내주식-008]", "주식현재가 체결[v1_국내주식-009]"]
    row_index, header = extract_api_parameters(apis_name_list[1])
    # print(row_index, header)
    summary = api_row_position(row_index)
